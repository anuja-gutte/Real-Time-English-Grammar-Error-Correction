...
//     // Clear previous suggestion while typing new content
...
//     // Split into sentences (works for full and short/incomplete text)
...
//     // If block has multiple sentences, check whole block
//     // otherwise check the short sentence itself
//     const unitToCheck = sentences.length > 1 ? lastBlock : lastSentence;

//     // Avoid repeated same request
//     if (!unitToCheck || unitToCheck === checkedUnit) return;

//     // Minimum length for grammar checking
...
//       if (
//         corrected &&
//         corrected.toLowerCase() !== unit.trim().toLowerCase()
//       ) {
//         const suggestionObj = {
//           original: unit,
//           corrected,
...
//         // Auto-correct after delay if enabled
//         if (autoCorrect) {
...
export default function GrammarEditor() {
  ...
  const handleChange = (e) => {
    ...
    // Clear old popup when typing new content
    if (suggestion) setSuggestion(null);
  ...
  const applyCorrection = (suggestionToApply) => {
    ...
    setText((prev) => {
      // SAFETY CHECK:
      // only replace if the exact original sentence still exists at that index
